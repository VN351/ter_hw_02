...
2.  Били исправлены несколько ошибок в коде:
    - Изменена строка ssh-keys для передачи ключа на:
      ```
    ...
    - изменено значение core на 2, так как это минимальное значение.
...
1.  Часть кода variables.tf
    ...
    variable "ssh_key" {
      ...
      default = {
        "serial-port-enable" = "1"
        "ssh-keys"           = "ubuntu:ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIC8bDGbiyUNa2k07/T9jlaRKD1/4wqljOvFJOD nevzorovvlad@mail.ru"
      }

    }
    ```
    При такой конфигурации при выполнении terraform plan требовал изменения, так как пользовался конструкцией
    ```
    metadata = {
      serial-port-enable = 1
      ssh-keys           = "ubuntu:${file(var.vms_ssh_root_key)}"
    ...
    ```
    ![alt text](https://github.com/VN351/ter_hw_02/raw/main/images/task-6-1.png)
    
    Решил задачу без изменений в terraform plan немного по другому
    в variables.tf добавил
    ```
    variable "vms_ssh_root_key_1" {
    ...
    vms_ssh_root_key = {
        "serial-port-enable" = "1"
        "ssh-keys"           = "${var.ssh_username}:${file(var.vms_ssh_root_key_1)}"
    }
    ```
    В main.tf находится 3 варианта для передачеи metadata
    ```
    ...
    ```
    ![alt text](https://github.com/VN351/ter_hw_02/raw/main/images/task-6-2.png) 
